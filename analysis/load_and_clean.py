...
def cleanup(input_df):
    ...
    if 'Price' in mydf2.columns:
        mydf2.rename(columns={'Price':'Close'}, inplace=True)

    if 'High' in mydf2.columns:
...
def currencies_no_trading(dfin, target_hour=None):
    if target_hour is None:
        print("assumign daily data in currencies_no_trading")
        tmp_df = dfin.copy()
...
def load_and_clean(interval):
    if interval == 'hourly':
        file_list = ['2024.t2.year.csv', '2023.q4.csv', '2023.q3.csv', '2023.q2.csv', '2023.q1.csv', '2022.year.csv',
                     '2022b.year.csv', 'backfill.1.csv', 'backfill.2.csv']
        hours=24
    elif interval == 'daily':
    ...
    for file in file_list:
        tmp_in_df = pd.read_csv("/Users/george/PycharmProjects/cryptohistory1/ddownload/data/" + file)
        if in_df is None:
    ...
    check_missing_records(tmp_df2, hours=hours)
    ...
    if interval == 'daily':
        ...
        currencies_no_trading(tmp_df2, target_hour=10)
