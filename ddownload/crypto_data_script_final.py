...
exchanges = input('Specify the exchanges(Format:binance,coinbase,kraken)=')
exchanges = exchanges.split(",")
for i in range(len(exchanges)):
    exchanges[i] = exchanges[i].capitalize()
...
all_exchanges = list(api_endpoints.keys())

for exchange in all_exchanges:
    if exchange not in exchanges:
        del api_endpoints[exchange]

kraken_interval, binance_interval, coinbase_interval = "1", "1m", "60"
...
exchange_to_interval = {"Coinbase": coinbase_interval,
                        "Kraken": kraken_interval,
                        "Binance": binance_interval}

exchange_to_interval_in_seconds = {"Coinbase": int(coinbase_interval),
...
def batch_request_by_limits(start_datetime, end_datetime, max_items, api_endpoint, url_str, symbol, time_interval):
    ...
    interval_in_seconds = exchange_to_interval_in_seconds[api_endpoint]
    ...
    if api_endpoint == "Kraken":
        if total_items > max_items:
            ...
            global date_kraken_glob
            date_kraken_glob = start_date_kraken
...
def create_file(rows, api_endpoint):
    if api_endpoint == "Kraken" and date_kraken_glob is not None:
        start = date_kraken_glob.strftime("%Y-%m-%dT%H:%M:%S")
        file_name = api_endpoint + "_" + exchange_to_interval[api_endpoint] + "_" + start + "_" + end_date
    else:
        file_name = api_endpoint + "_" + exchange_to_interval[api_endpoint] + "_" + start_date + "_" + end_date
...
for api_endpoint, url_str in api_endpoints.items():
    ...
    for coin in API_SYMBOL_MAPPIN.keys():
        ...
        data = batch_request_by_limits(start_date, end_date, ENDPOINT_TO_LIMIT[api_endpoint],
                                       api_endpoint, url_str, symbol, exchange_to_interval[api_endpoint])
